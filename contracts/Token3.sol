...
import "./Token3DividendTracker.sol";
import "./BaseToken.sol";

contract Token3 is Initializable, ERC20Upgradeable, AccessControlUpgradeable, BaseToken {
    ...
    Token3DividendTracker public dividendTracker;

    address public rewardToken;

    uint256 public swapTokensAtAmount;
    uint256 public maxLimitSell;
    ...
    mapping(address => bool) private isExcludedFromMaxLimitSell;
    ...
    event ExcludeFromMaxLimitSell(address account,bool excluded);
    function initialize(
        string memory name_,
        string memory symbol_,
        uint256 totalSupply_,
        address[4] memory addrs, // reward, router, marketing wallet, dividendTracker
        uint256[4] memory feeSettings, // rewards, liquidity, marketing, max limit to sell
        ...
        dividendTracker = Token3DividendTracker(
        ...
        maxLimitSell=feeSettings[3];
        IPancakeRouter02 _uniswapV2Router = IPancakeRouter02(addrs[1]);
    ...
    function setSwapTokensAtAmount(uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        swapTokensAtAmount = amount;
    }

    function updateDividendTracker(address newAddress) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            newAddress != address(dividendTracker),
            "BABYTOKEN: The dividend tracker already has that address"
        );

        Token3DividendTracker newDividendTracker = Token3DividendTracker(
            payable(newAddress)
        );
        ...
        newDividendTracker.excludeFromDividends(admin);
    ...
    function updateUniswapV2Router(address newAddress) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
    ...
    function excludeFromMaxLimitSell(address account, bool excluded) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            isExcludedFromMaxLimitSell[account] != excluded,
            "BABYTOKEN: Account is already the value of 'excluded'"
        );
        isExcludedFromMaxLimitSell[account] = excluded;

        emit ExcludeFromMaxLimitSell(account, excluded);
    }
    ...
    function setMarketingWallet(address payable wallet) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _marketingWalletAddress = wallet;
    }

    function setTokenRewardsFee(uint256 value) external onlyRole(DEFAULT_ADMIN_ROLE) {
        tokenRewardsFee = value;
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee);
        require(totalFees <= 25, "Total fee is over 25%");
    }

    function setLiquiditFee(uint256 value) external onlyRole(DEFAULT_ADMIN_ROLE) {
        liquidityFee = value;
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee);
        require(totalFees <= 25, "Total fee is over 25%");
    }

    function setMarketingFee(uint256 value) external onlyRole(DEFAULT_ADMIN_ROLE) {
        marketingFee = value;
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee);
        require(totalFees <= 25, "Total fee is over 25%");
    }
    ...
    function updateGasForProcessing(uint256 newValue) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
    ...
    function updateClaimWait(uint256 claimWait) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function excludeFromDividends(address account) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function _transfer(
        ...
        if(automatedMarketMakerPairs[to] && !isExcludedFromMaxLimitSell[from]){
            require(amount<maxLimitSell, "More than limit to sell");
        }
        if (
            canSwap &&
            !swapping &&
