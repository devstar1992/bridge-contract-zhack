...
import "./interfaces/IACRN.sol";
import "./interfaces/IsACRN.sol";
import "./interfaces/IBondingCalculator.sol";
import "./interfaces/ITreasury.sol";

import "./types/AcranaAccessControlled.sol";

contract AcranaTreasury is AcranaAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    constructor(
        ...
        uint256 _timelock,
        address _authority
    ) AcranaAccessControlled(IAcranaAuthority(_authority)) {
    ...
    /**
     * @notice allow approved address to deposit an asset for ACRN
    ...
    function deposit(
        ...
        // mint ACRN needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        ACRN.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn ACRN for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        ACRN.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new ACRN using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        ACRN.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or ACRN from the treasury, using sACRN as collateral. This might allow an
     * sACRN holder to provide ACRN liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(ACRN)) {
            require(permissions[STATUS.ACRNDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sACRN.changeDebt(value, msg.sender, true);
        require(sACRN.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(ACRN)) {
            ACRN.mint(msg.sender, value);
            acrnDebt = acrnDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sACRN.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with ACRN
     * @param _amount uint256
     */
    function repayDebtWithACRN(uint256 _amount) external {
        require(
            permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.ACRNDEBTOR][msg.sender],
            notApproved
        );
        ACRN.burnFrom(msg.sender, _amount);
        sACRN.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        acrnDebt = acrnDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(ACRN), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SACRN) {
            sACRN = IsACRN(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SACRN) {
            // 9
            sACRN = IsACRN(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(ACRN.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns ACRN valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(ACRN)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return ACRN.totalSupply() - acrnDebt;
    }
}
