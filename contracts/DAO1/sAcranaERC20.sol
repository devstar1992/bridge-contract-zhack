...
contract sAcrana is IsACRN, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgACRN public gACRN; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked ACRN", "sACRN", 9) ERC20Permit("Staked ACRN") {
    ...
    function setgACRN(address _gACRN) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gACRN) == address(0), "gACRN:  gACRN already set");
        require(_gACRN != address(0), "gACRN:  gACRN is not a valid contract");
        gACRN = IgACRN(_gACRN);
    }
    ...
    /**
        @notice increases rACRN supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sACRN of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sACRN
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sACRN: insufficient balance");
    }
    ...
    // toG converts an sACRN balance to gACRN terms. gACRN is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gACRN.balanceTo(amount);
    }

    // fromG converts a gACRN balance to sACRN terms. sACRN is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gACRN.balanceFrom(amount);
    }

    // Staking contract holds excess sACRN
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gACRN.balanceFrom(IERC20(address(gACRN)).totalSupply())).add(
