...
import {IsACRN} from "../interfaces/IsACRN.sol";
import {SafeERC20} from "../libraries/SafeERC20.sol";
import {IYieldDirector} from "../interfaces/IYieldDirector.sol";
import {AcranaAccessControlled, IAcranaAuthority} from "../types/AcranaAccessControlled.sol";

/**
    @title YieldDirector (codename Tyche) 
    @notice This contract allows donors to deposit their sACRN and donate their rebases
            to any address. Donors will be able to withdraw their principal
            sACRN at any time. Donation recipients can also redeem accrued rebases at any time.
 */
contract YieldDirector is IYieldDirector, AcranaAccessControlled {
    using SafeERC20 for IERC20;

    uint256 private constant MAX_UINT256 = type(uint256).max;

    address public immutable sACRN;
    ...
    struct DonationInfo {
        ...
        uint256 carry; // Amount of sACRN accumulated over on deposit/withdraw
    ...
    constructor(address sAcrn_, address authority_) AcranaAccessControlled(IAcranaAuthority(authority_)) {
        require(sAcrn_ != address(0), "Invalid address for sACRN");
    ...
    /**
        @notice Deposit sACRN, records sender address and assign rebases to recipient
        @param amount_ Amount of sACRN debt issued from donor to recipient
    ...
    */
    function deposit(uint256 amount_, address recipient_) external override {
        ...
        IERC20(sACRN).safeTransferFrom(msg.sender, address(this), amount_);

        uint256 index = IsACRN(sACRN).index();
    ...
    /**
        @notice Withdraw donor's sACRN from vault and subtracts debt from recipient
     */
    function withdraw(uint256 amount_, address recipient_) external override {
        ...
        IERC20(sACRN).safeTransfer(msg.sender, amount_);
    ...
    function withdrawAll() external override {
        ...
        uint256 sAcrnIndex = IsACRN(sACRN).index();
        uint256 total = 0;
        ...
        for (uint256 index = 0; index < donationsLength; index++) {
            ...
            recipient.agnosticDebt = _toAgnostic(recipient.totalDebt + recipient.carry);
            recipient.indexAtLastChange = sAcrnIndex;
        ...
        IERC20(sACRN).safeTransfer(msg.sender, total);

        emit AllWithdrawn(msg.sender, total);
    }

    /**
        @notice Get deposited sACRN amount for specific recipient
     */
    ...
    /**
        @notice Return total amount of donor's sACRN deposited
     */
    ...
    /**
        @notice Return total amount of sACRN donated to recipient since last full withdrawal
     */
    ...
    /**
        @notice Return total amount of sACRN donated from donor since last full withdrawal
     */
    ...
    /**
        @notice Get redeemable sACRN balance of a recipient address
     */
    ...
    /**
        @notice Redeem recipient's full donated amount of sACRN at current index
        @dev Note that a recipient redeeming their vault shares effectively pays back all
             sACRN debt to donors at the time of redeem. Any future incurred debt will
    ...
    function redeem() external override {
        ...
        recipient.carry = 0;
        recipient.indexAtLastChange = IsACRN(sACRN).index();

        IERC20(sACRN).safeTransfer(msg.sender, redeemable);
    ...
    /**
        @notice Get accumulated sACRN since last time agnostic value changed.
     */
    ...
    /**
        @notice Convert flat sACRN value to agnostic value at current index
        @dev Agnostic value earns rebases. Agnostic value is amount / rebase_index.
             1e9 is because sACRN has 9 decimals.
     */
    function _toAgnostic(uint256 amount_) internal view returns (uint256) {
        return (amount_ * 1e9) / (IsACRN(sACRN).index());
    }

    /**
        @notice Convert agnostic value at current index to flat sACRN value
        @dev Agnostic value earns rebases. Agnostic value is amount / rebase_index.
             1e9 is because sACRN has 9 decimals.
     */
    function _fromAgnostic(uint256 amount_) internal view returns (uint256) {
        return (amount_ * (IsACRN(sACRN).index())) / 1e9;
    }

    /**
        @notice Convert flat sACRN value to agnostic value at a given index value
        @dev Agnostic value earns rebases. Agnostic value is amount / rebase_index.
             1e9 is because sACRN has 9 decimals.
     */
