...
interface IClaim {
    struct Term {
        ...
        uint256 max; // maximum nominal ACRN amount can claim
    }
...
/**
 *  This contract allows Acrana genesis contributors to claim ACRN. It has been
...
contract GenesisClaim is Ownable {
    ...
    // claim token
    IERC20 internal immutable acrn = IERC20(0x64aa3364F17a4D01c6f1751Fd97C2BD3D7e7f1D5);
    // payment token
    IERC20 internal immutable busd = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    // mints claim token
    ITreasury internal immutable treasury = ITreasury(0x9A315BdF513367C0377FB36545857d12e85813Ef);
    // stake ACRN for sACRN
    ...
    IgACRN internal immutable gACRN = IgACRN(0x0ab87046fBb341D058F17CBC4c1133F25a20a52f);
    ...
    /**
     * @notice allows wallet to claim ACRN
     * @param _to address
     * @param _amount uint256
     */
    function claim(address _to, uint256 _amount) external {
        acrn.safeTransfer(_to, _claim(_amount));
    }

    /**
     * @notice allows wallet to claim ACRN and stake. set _claim = true if warmup is 0.
     * @param _to address
     * @param _amount uint256
    ...
    /**
     * @notice logic for claiming ACRN
     * @param _amount uint256
    ...
    function _claim(uint256 _amount) internal returns (uint256 toSend_) {
        ...
        toSend_ = treasury.deposit(_amount, address(busd), 0);
        ...
        if (useStatic) {
            terms[msg.sender].gClaimed = info.gClaimed.add(gACRN.balanceTo(toSend_.mul(9).div(10)));
            terms[msg.sender].claimed = info.claimed.add(toSend_.div(10));
        } else terms[msg.sender].gClaimed = info.gClaimed.add(gACRN.balanceTo(toSend_));
    }
    ...
    function approve() external {
        acrn.approve(address(staking), 1e33);
        busd.approve(address(treasury), 1e33);
    }

    /* ========== VIEW FUNCTIONS ========== */

    /**
     * @notice view ACRN claimable for address. BUSD decimals (18).
    ...
    /**
     * @notice view ACRN claimed by address. ACRN decimals (9).
     * @param _address address
     * @return uint256
     */
    function claimed(address _address) public view returns (uint256) {
        return gACRN.balanceFrom(terms[_address].gClaimed).add(terms[_address].claimed);
    }

    /**
     * @notice view circulating supply of ACRN
     * @notice calculated as total supply minus DAO holdings
     * @return uint256
     */
    function circulatingSupply() public view returns (uint256) {
        return treasury.baseSupply().sub(acrn.balanceOf(dao));
    }
